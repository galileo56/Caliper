module NRQCDClass
  ...
  Chi2MinAlphaMbNRQCD, Chi2MinAlphaNRQCD
...
!ccccccccccccccc
  ...
  real (dp) function EnFitter(self, iter, n, order, mu, R, lambda, method, counting)
    class (NRQCD)      , intent(inout) :: self
    character (len = *), intent(in)    :: method, iter, counting
    integer            , intent(in)    :: order, n
    ...
    if ( iter(:10) == 'FixedOrder' ) then
      ...
      list = self%En(order, mu, R, lambda, method, counting)
      EnFitter = sum(  list( :min(4,n) )  )

    else if ( iter(:8) == 'expanded' ) then

      list = self%EnExpand(order, mu, R, 1._dp, lambda, method, counting)

      list(0) = 0.5_dp - list(0); list(1:) = 2 * list(1:)

      EnFitter = 2 * ( list(0) + self%mH )/(   1 + sum(  list( 1:min(4,n) )  )   )
    end if

    if ( iter(:9) /= 'iterative' ) return

    EnFitter = FindRoot(2 * self%mH)

    do i = 1, 20
      a = FindRoot(EnFitter);  if ( abs(a - EnFitter) < 1e-10_dp ) exit
      EnFitter = a
    end do
  ...
  contains
    ...
    real (dp) function FindRoot(mass)
      ...
      list = self%MassIter(order, mu, R, mass, lambda, method, counting)
      list(0) = mass/2 - list(0); list(1:) = 2 * list(1:)/mass

      FindRoot = 2 * ( self%mH + list(0) )/(   1 + sum(  list( 1:min(4,n) )  )   )

    end function FindRoot

  end function EnFitter
...
!ccccccccccccccc
  ...
  pure real (dp) function c3(nl, n, l, j, s)
    ...
    if (n == 1) then
      ...
      if (l == 0) then
        c3 = 1928.7613186214833_dp - 418.0026977201289_dp * nl   + &
      ...
      if (l == 0) then
        ...
        c3 = 1901.5001792689523_dp + jj * (4.079182167775893_dp      - &
        0.11282798880462161_dp * nl) + jj2 * (-0.1299105555452488_dp + &
        0.0040335703497889385_dp * nl) - 402.34686857080703_dp * nl  + &
      ...
      if (l == 0) then
        ...
        c3 = 1997.2054361449605_dp + jj * (3.958438263203731_dp          - &
        0.14118158483048285_dp * nl) + jj2 * (-0.12522904650665853_dp    + &
        0.004792696300685975_dp * nl) - 431.61050895589005_dp * nl        + &
        ...
        c3 = 1882.18394244926_dp + jj * (1.1136805133484151_dp             - &
        0.02558450978851483_dp * nl) + jj2 * (-0.017109601029735064_dp      + &
        0.00044023804726528275_dp * nl) - 396.6288446058971_dp * nl        + &
...
real (dp) function Chi2NRQCD(UpsilonList, datalist, dim, iter, order, n, &
muList, RList, ndim, lambda, method, counting)
  ...
  do i = 1, dim
    ...
    MassList(i) = UpsilonList(i)%EnFitter(iter, n, order, mu, R, lambda, method, counting)
...
function Chi2MinNRQCD(UpsilonList, datalist, dim, iter, order, n, mH, muList, &
RList, ndim, lambda, method, counting) result(res)
...
delta_tol = 1.e-7_dp; delta_init = 1.e-3_dp;  k_max = 100000000; x0(1) = mH
...
contains
  ...
  real (dp) function chi2(nn,mass)
    ...
    chi2 = Chi2NRQCD(UpsilonList, datalist, dim, iter, order, n, muList, RList, &
    ndim, lambda, method, counting)
...
function Chi2MinAlphaNRQCD(UpsilonList, datalist, dim, iter, order, n, alpha, &
...
delta_tol = 1.e-7_dp; delta_init = 1.e-3_dp;  k_max = 100000000; x0(1) = alpha
...
call f90compass_search( chi2, 1, x0, delta_tol, delta_init, k_max, x, res(3), ktot )
  ...
  do i = 0, 1
    list(1,i+1)= x(1) + 0.001_dp * i; list(2,i + 1) = chi2(1, [list(1,i + 1)] )
    list(1,-i) = x(1) - 0.001_dp * i; list(2,   -i) = chi2(1, [list(1,  - i)] )
  end do
...
contains
  ...
  real (dp) function chi2(nn,mass)
    ...
    if ( mass(1) < 2 * alpha/3 .or. mass(1) > 3 * alpha/2) then
      chi2 = 1.e300_dp; return
    end if

    do j = 1, dim
      call UpsilonList(j)%setAlpha( mass(1) )
    end do
...
function Chi2MinAlphaMbNRQCD(UpsilonList, datalist, dim, iter, order, n, mH, &
alpha, muList, RList, ndim, lambda, method, counting) result(res)
...
delta_tol = 1.e-7_dp; delta_init = 1.e-3_dp;  k_max = 100000000; x0 = [mH, alpha]
...
contains
  ...
  real (dp) function chi2(nn, x)
    ...
    chi2 = Chi2NRQCD(UpsilonList, datalist, dim, iter, order, n, muList, RList, ndim, &
    lambda, method, counting)

  end function chi2

end function Chi2MinAlphaMbNRQCD
