...
!  Modified:
!
!    07 June 2013  // 13 July 2016
!
!  Author:
!
!    John Burkardt // Vicent Mateu
!
!  Reference:
!
!    Carl deBoor,
...
!    the cubic spline.
  ...
  integer, intent(in) :: n
  real ( kind = 8 ), intent(in), dimension(n) :: t, y
  integer, intent(in) :: ibcbeg, ibcend
  real ( kind = 8 ), intent(in) :: ybcbeg, ybcend

  real ( kind = 8 ), intent(out), dimension(n) :: ypp

  real ( kind = 8 ), dimension(n) :: a1, a2, a3, a4, a5, b
  integer :: i
...
!  Set the first equation.
  ...
  if ( ibcbeg == 0 ) then
    ...
    a3(1) = ( t(2) - t(1) ) / 3;  a4(1) = ( t(2) - t(1) ) / 6
  else if ( ibcbeg == 2 ) then
    b(1) = ybcbeg;  a3(1) = 1;  a4(1) = 0
  else if ( ibcbeg == 3 ) then
    b(1) = 0;  a3(1) = - ( t(3) - t(2) )
    a4(1) = ( t(3) - t(1) );  a5(1) = - ( t(2) - t(1) )
  else
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'SPLINE_CUBIC_SET - Fatal error!'
...
!  Set the intermediate equations.
  ...
  do i = 2, n - 1
    ...
    a2(i) = ( t(i+1) - t(i)   ) / 6;  a3(i) = ( t(i+1) - t(i-1) ) / 3
    a4(i) = ( t(i)   - t(i-1) ) / 6
  end do
...
!  Set the last equation.
  ...
  if ( ibcend == 0 ) then
    ...
    a2(n) = ( t(n) - t(n-1) ) / 6;  a3(n) = ( t(n) - t(n-1) ) / 3
  else if ( ibcend == 2 ) then
    b(n) = ybcend;  a2(n) = 0;  a3(n) = 1
  else if ( ibcend == 3 ) then
    b(n) = 0;  a1(n) = - ( t(n) - t(n-1) )
    a2(n) =   ( t(n) - t(n-2) );  a3(n) = - ( t(n-1) - t(n-2) )
  else
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'SPLINE_CUBIC_SET - Fatal error!'
...
!    its first two derivatives at TVAL.
  ...
  real ( kind = 8 ), intent(in), dimension(n) :: t, y, ypp
  real ( kind = 8 ), intent(out) :: yppval, ypval, yval
...
!  Evaluate the polynomial.
  ...
  dt = tval - t(left);  h = t(right) - t(left)

  yval = y(left) + dt * ( ( y(right) - y(left) ) / h &
       - ( ypp(right) / 6 + ypp(left) / 3 ) * h  + dt * ( ypp(left) / 2 &
       + dt * ( ( ypp(right) - ypp(left) ) / ( 6 * h ) ) ) )

  ypval = ( y(right) - y(left) ) / h - ( ypp(right) / 6 + ypp(left) / 3 ) * h &
          + dt * ( ypp(left) + dt * ( ypp(right) - ypp(left) ) / h / 2 )
...
!    Output, real ( kind = 8 ) X(N), the solution of the linear system.
  ...
  integer, intent(in) :: n
  real ( kind = 8 ), intent(inout), dimension(n) :: a1, a2, a3, a4, a5, b
  ...
  do i = 2, n - 1
    xmult = a2(i) / a3(i-1);  a3(i) = a3(i) - xmult * a4(i-1)
    a4(i) = a4(i) - xmult * a5(i-1);  b(i) = b(i) - xmult * b(i-1)
    xmult = a1(i+1) / a3(i-1);  a2(i+1) = a2(i+1) - xmult * a4(i-1)
    a3(i+1) = a3(i+1) - xmult * a5(i-1);  b(i+1) = b(i+1) - xmult * b(i-1)
  end do

  xmult = a2(n) / a3(n-1);  a3(n) = a3(n) - xmult * a4(n-1)
  x(n) = ( b(n) - xmult * b(n-1) ) / a3(n); x(n-1) = ( b(n-1) - a4(n-1) * x(n) ) / a3(n-1)
