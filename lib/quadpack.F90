...
module QuadPack
  ...
  public :: qag, qags, qagp, qagi, qawo, qawf, qaws, qawc, qng
...
!    which evaluates the integrand function.
!
!    Input, real (dp) BOUND, the value of the finite endpoint of the integration
...
!  Parameters:
!
!    Input, external real (dp) F, the name of the function routine,
!    of the form
!      function f ( x )
!      real (dp) f
!      real (dp) x
...
!
  ...
  if ( 1.0E-02 > (result/area) .or. (result/area) > 1.0D+02 .or. &
    errsum > abs(area) ) then
    ier = 6
  end if
...
!
  ...
  real (dp) ::  a, abseps, abserr, area, area1, area12, area2, a1, a2, b, b1, b2, &
                        correc, defabs, defab1, defab2, dres, epsabs, epsrel, erlarg, &
                        erlast, errbnd, errmax, error1, error2, erro12, errsum, ertest
  real (dp), dimension(limit) :: alist, blist, elist
...
110 continue
  ...
  if ( 1.0E-02 > (result/area) .or. (result/area) > 1.0D+02 &
...
!    Input, real (dp) A, B, the limits of integration.
!
!    Input, real (dp) C, a parameter in the weight function, which must
...
!    Input, real (dp) A, B, the limits of integration.
!
!    Input, real (dp) C, the parameter in the weight function.
!
!    Output, real (dp) RESULT, the estimated value of the integral.
...
!    Output, integer NEVAL, the number of times the integral was evaluated.
!
!    Output, real (dp) RESABS, approximation to the integral J.
!
!    Output, real (dp) RESASC, approximation to the integral of abs(F-I/(B-A)).
!
!         on entry and return
!           momcom - integer
  ...
  if ( abs(parint) <= 2.4D+01 ) then
    ...
    do k = 3, 12
      ...
      chebmo(m,2*k) = -sinpar/(an*(2.0D+00*an-2.0D+00)) &
                 -2.5E-01*parint*(v(k+1)/an-v(k)/(an-1.0D+00))
    end do
!
!  Compute the Chebyshev moments by means of forward recursion.
!
...
!    Input, real (dp) A, B, the limits of integration.
!
!    Input, real (dp) BL, BR, the lower and upper limits of integration.
!    A <= BL < BR <= B.
!
!    Input, real (dp) ALFA, BETA, parameters in the weight function.
!
!    Input, real (dp) RI(25), RJ(25), RG(25), RH(25), modified Chebyshev moments
!    for the application of the generalized Clenshaw-Curtis method,
!    computed in QMOMO.
!
!    Output, real (dp) RESULT, the estimated value of the integral, computed by
...
!    Output, real (dp) ABSERR, an estimate of || I - RESULT ||.
!
!    Output, real (dp) RESASC, approximation to the integral of abs(F*W-I/(B-A)).
!
...
!  Parameters:
!
!    Input, real (dp) X(11), contains the values of COS(K*PI/24), for K = 1 to 11.
!
!    Input/output, real (dp) FVAL(25), the function values at the points
...
!
!    Output, real (dp) CHEB12(13), the Chebyshev coefficients for degree 12.
!
!    Output, real (dp) CHEB24(25), the Chebyshev coefficients for degree 24.
!
...
!
!    Input/output, real (dp) EPSTAB(52), the two lower diagonals of the triangular
!    epsilon table.  The elements are numbered starting at the right-hand
!    corner of the triangle.
!
!    Output, real (dp) RESULT, the estimated value of the integral.
!
!    Output, real (dp) ABSERR, estimate of the absolute error computed from
!    RESULT and the 3 previous results.
!
!    ?, real (dp) RES3LA(3), the last 3 results.
!
...
!    Input, real (dp) A, B, the limits of integration.
!
!    Input, real (dp) OMEGA, the multiplier of X in the weight function.
!
...
!
  ...
  if ( 5.0E-01*abs(b-a)*domega <= 2.0D+00) then
    numrl2 = 1
    extall = .true.
    rlist2(1) = result
  end if

  if ( 2.5E-01 * abs(b-a) * domega <= 2.0D+00 ) then
    extall = .true.
  end if
...
!
    ...
    if ( 2.5E-01*width*domega > 2.0D+00 ) then
      cycle
    end if
...
!
  data xgk(1),xgk(2),xgk(3),xgk(4),xgk(5),xgk(6),xgk(7),xgk(8)/ &
       ...
       8.648644233597691E-01,     7.415311855993944E-01, &
       5.860872354676911E-01,     4.058451513773972E-01, &
       2.077849550078985E-01,     0.0000000000000000D+00/
  ...
  data wg(1),wg(2),wg(3),wg(4),wg(5),wg(6),wg(7),wg(8)/ &
       0.0000000000000000D+00,     1.294849661688697E-01, &
       0.0000000000000000D+00,     2.797053914892767E-01, &
       0.0000000000000000D+00,     3.818300505051189E-01, &
       0.0000000000000000D+00,     4.179591836734694E-01/
...
!
  data xgk(1),xgk(2),xgk(3),xgk(4),xgk(5),xgk(6),xgk(7),xgk(8)/ &
       ...
       8.648644233597691E-01,     7.415311855993944E-01, &
       5.860872354676911E-01,     4.058451513773972E-01, &
       2.077849550789850E-01,     0.000000000000000D+00/
...
!  Parameters:
!
!    Input, real (dp) ALFA, a parameter in the weight function w(x), ALFA > -1.
!
!    Input, real (dp) BETA, a parameter in the weight function w(x), BETA > -1.
!
!           ri     - real
...
!    largest error.
!
!    Output, real (dp) ERMAX, the NRMAX-th largest error = ELIST(MAXERR).
!
!    Input, real (dp) ELIST(LIMIT), contains the error estimates.
!
...
!  Parameters:
!
!    Input, real (dp) X, the point at which the weight function is evaluated.
!
!    Input, real (dp) C, the location of the singularity.
!
!    Input, real (dp) P2, P3, P4, parameters that are not used.
!
!    Input, integer KP, a parameter that is not used.
!
!    Output, real (dp) QWGTC, the value of the weight function at X.
!
...
!    Input, real (dp) X, the point at which the weight function is evaluated.
!
!    Input, real (dp) OMEGA, the factor multiplying X.
!
!    Input, real (dp) P2, P3, P4, parameters that are not used.
!
...
!    2, W(X) = sin ( OMEGA * X )
!
!    Output, real (dp) QWGTO, the value of the weight function at X.
!
...
!    Input, real (dp) X, the point at which the weight function is evaluated.
!
!    Input, real (dp) A, B, the endpoints of the integration interval.
!
!    Input, real (dp) ALFA, BETA, exponents that occur in the weight function.
!
!    Input, integer INTEGR, specifies which weight function is used:
!    1. W(X) = (X-A)**ALFA * (B-X)**BETA
!    2, W(X) = (X-A)**ALFA * (B-X)**BETA * log (X-A)
...
!    4, W(X) = (X-A)**ALFA * (B-X)**BETA * log (X-A) * log(B-X)
!
!    Output, real (dp) QWGTS, the value of the weight function at X.
!
