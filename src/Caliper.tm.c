...
# line 2502 "/Users/vmateu/GitHub/Caliper/src/Caliper.tm"
#include "mathlink.h"
#include "ftypes.h"
#include <stdio.h>
#include <unistd.h>
...
# line 3811 "/Users/vmateu/GitHub/Caliper/src/Caliper.tm.c"
...
static struct func {
	...
	const char  *f_name;
		...
		{15, 0, _tr187, "rqcd" },
		{16, 0, _tr188, "rexp" },
		{14, 0, _tr189, "lambdaqcd" },
		{ 5, 0, _tr190, "kernels" },
		{ 2, 0, _tr191, "gammaderlist" },
		{ 2, 0, _tr192, "polygamma" },
		{ 7, 0, _tr193, "nglkernels" },
		{ 4, 0, _tr194, "nglintegral" },
		{ 5, 0, _tr195, "ngldoubleintegral" }
		};
...
static const char* evalstrs[] = {
	...
	(const char*)0,
	"RExp::usage = \"RExp[scheme, runAlpha, runMass, ordMass, ord1S, R",
	"1S, order, method, lambda, gt, Mz, aMz, mT, mu, nu, Q] computes ",
...
#endif
{
	...
	if (_res) _res = _definepattern(mlp, (char *)"LambdaQCD[scheme_, order_, runAlpha_, run_, nf_, Mz_, aMz_, mT_, muT_,                  mB_, muB_, mC_, muC_, mu_]", (char *)"{scheme, order, runAlpha, run, nf, Mz, aMz, mT, muT, mB, muB, mC, muC, mu}", 189);
	if (_res) _res = _definepattern(mlp, (char *)"Kernel[n_, width_, w_, mu_, p_]", (char *)"{n, width, w, mu, p}", 190);
	if (_res) _res = _definepattern(mlp, (char *)"GammaDerList[n_, w_]", (char *)"{n, w}", 191);
	if (_res) _res = _definepattern(mlp, (char *)"polyGamma[n_, w_]", (char *)"{n, w}", 192);
	if (_res) _res = _definepattern(mlp, (char *)"NGLKernel[n_, n1_, n2_, width_, w_, mu_, p_]", (char *)"{n, n1, n2, width, w, mu, p}", 193);
	if (_res) _res = _definepattern(mlp, (char *)"NGLIntegral[nf_, pow_, w1_, w2_]", (char *)"{nf, pow, w1, w2}", 194);
	if (_res) _res = _definepattern(mlp, (char *)"NGLDoubleIntegral[nf_, pow_, w1_, w2_, r_]", (char *)"{nf, pow, w1, w2, r}", 195);
...
#endif
{
	return _MLDoCallPacket( mlp, _tramps, 196);
} /* MLDoCallPacket */
